...
document.querySelector("#sellForm").onsubmit=e=>{
 e.preventDefault();const file=document.querySelector("#image").files[0];if(!file)return;
 if(file.size>5*1024*1024){document.querySelector("#sellMessage").textContent="Image trop volumineuse (5 Mo maximum dans cette démo).";return}
 const reader=new FileReader();reader.onload=()=>{
  items.push({id:Date.now(),title:document.querySelector("#title").value,description:document.querySelector("#description").value,price:Number(document.querySelector("#price").value),image:reader.result,paymentType:document.querySelector("#paymentType").value,paymentValue:document.querySelector("#paymentValue").value,status:"pending",unlocked:false,proof:null});
  save();e.target.reset();document.querySelector("#sellMessage").textContent="✓ Soumission envoyée. Elle attend maintenant une vérification.";renderAdmin();
...
window.openItem=id=>{
 ...
 ["modalPayment","proofForm"].forEach(id=>document.getElementById(id)?.remove());
